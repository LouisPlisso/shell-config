...
xkbcomp  -w 0 -R/usr/share/X11/xkb/  ${INSTALL_DIR}/keymap  "$DISPLAY" 
# set it as default
xkbcomp -xkb $DISPLAY
...
if /bin/chmod 000 smbcredentials; then
	...
	sudo /bin/ln -s ${INSTALL_DIR}/smbcredentials /root/.smbcredentials
fi
...
for file in profile inputrc vimrc gvimrc Xdefaults xmonad xmobarrc cabal XCompose keynavrc dircolors lxdvdrip.conf abcde.conf vimperator vim vimperator/vimperatorrc vimperator/muttatorrc xpdfrc matplotlib/matplotlibrc gitconfig ctags; do

...
import XMonad.Hooks.ManageDocks (checkDock,avoidStruts,manageDocks)
import XMonad.Hooks.ManageHelpers (isFullscreen,doFullFloat,isDialog,composeOne,(-?>))
...
main = do
    ...
    xmonad $ withUrgencyHook NoUrgencyHook $ ffConfig
        { manageHook = manageDocks <+> myManageHook 
            <+> manageHook defaultConfig -- <+> doF W.focusDown
...
--        , XMonad.focusFollowsMouse  = False
        ...
        , ((mod1Mask .|. shiftMask, xK_h), spawn "sudo pm-hibernate")
        ...
        , ((mod1Mask .|. shiftMask, xK_p), shellPrompt myXPConfig) -- instead of dmenu
        -- View next empty workspace
        , ((mod1Mask, xK_e), moveTo Next EmptyWS)            
        -- Move current window to next empty workspace
        , ((mod1Mask .|. shiftMask, xK_e), shiftTo' Next EmptyWS)          
        -- Restart xmonad
        , ((mod1Mask .|. shiftMask, xK_q), restart "xmonad" True)
        -- Quit xmonad
        , ((mod1Mask .|. controlMask, xK_Delete), io (exitWith ExitSuccess))
        -- Switch to next/previous workspace
        , ((mod1Mask, xK_x), nextWS)

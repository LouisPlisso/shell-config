...
-- M + F3               - Start dolphin                             --
-- M + F4               - Start chromium                            --
-- M + F5               - Start irssi in urxvt                      --
-- M + F6               - Start pidgin on last workspace            --
-- M + F7               - Start mutt in urxvt                       --
-- M + F8               - Start ncmpcpp in urxvt                    --
-- M + F10              - Start wicd-client
-- M + S + F4           - Start liferea                             --
-- M + S + F7           - Start abook in urxvt                      --
-- M + S + F8           - Start gmpc                                --
...
myWorkspaces            = clickable . (map dzenEscape) $ nWorkspaces 9 ["web", "irc", "im"]
  ...
  where nWorkspaces n []= map show [1 .. n]
        ...
        clickable l     = [ "^ca(1,xdotool key super+" ++ show n ++ ")"
                            -- ++ "^ca(2,xdotool key ctrl+super+" ++ show n ++ ")" -- pity :( dzen doesnt seem like nested ^ca()s
                            ++ ws
                            -- ++ "^ca()"
                            ++ "^ca()"
                          | (i,ws) <- zip [1..] l
...
myIcons layout
    ...
    | is "ReflectX IM ReflectX Grid"    = Just "layout-im.xbm"
...
myLayout = windowArrange . smartBorders . avoidStruts . toggleLayouts Full $
  ...
  where
    ...
    myIM        = reflectHoriz . withIM 0.15 (Role "buddy_list") . reflectHoriz $ Grid ||| Mirror Grid ||| Mirror tiled ||| tiled ||| Full -- Pidgin buddy list
